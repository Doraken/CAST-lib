...
# Date : 13/10/2006  
# generic # Subject : This lib is used to provide any function about web and download   #
...
MSG_DISPLAY "check" "0" "downloading [ ${Base_Param_URL}/${Base_Param_File_To_Get}${Base_Url_Parameter} ] "
if [ -e "${PWD_Recept_PKG}/${Base_Param_File_To_Get}" ]
   then
  	   MSG_DISPLAY "EdSMessage" "1" "Allready Downloaded" 
   else
        ...
        if [ ! -e "${PWD_Recept_PKG}/${Base_Param_File_To_Get}" ]
           then
               MSG_DISPLAY "EdEMessage" "1" "Failled"  
           else
               MSG_DISPLAY "EdSMessage" "1" "Success"  
         fi
  fi
